...
// phpcs:ignore
class JaipurUpdateMobileSummary
{
    private $dbConn;
    private $tableUtil;
    ...
    private function getTimeSpentTeamwise($sBranch)
    {
        ...
        $sQuery = "SELECT team_id, start_datetime, end_datetime FROM {$this->dbName}.tblvands_summary" .
            " WHERE dstatus = 0 AND activity_date = '$sDate' AND team_id IN (SELECT team_id FROM" .
    ...
    private function getROCSellInShopsCountTeamwise($sBranch)
    {
        ...
        $sQuery = "SELECT team_id, COUNT(pro_id) AS total FROM {$this->dbName}.{$this->respTable}" .
            " WHERE dstatus = 0 AND capture_date = '$sDate' AND ques_0 = 'ROC Delivery' AND ques_5 = 'Yes'" .
    ...
    private function getOtherCoveredShopsCountTeamwise($sBranch)
    {
        ...
        $sQuery = "SELECT team_id, COUNT(DISTINCT ques_2, ques_3) AS total FROM {$this->dbName}.{$this->respTable}" .
            ...
            " (SELECT team_id FROM {$this->dbName}.{$this->projectTeamTable} WHERE dstatus = 0 AND branch_id IN" .
            " ($sBranch)) {$this->otherSummaryCond} GROUP BY team_id";
        $this->dbConn->ExecuteSelectQuery($sQuery, $rsAction, $iActionRows);
        ...
        $sMtdQuery = "SELECT team_id, COUNT(DISTINCT ques_2, ques_3) AS total FROM {$this->dbName}.{$this->respTable}" .
            " WHERE dstatus = 0 AND capture_date LIKE '$sMonth' AND ques_0 = 'Other Outlet' AND team_id IN" .
            " (SELECT team_id FROM {$this->dbName}.{$this->projectTeamTable} WHERE dstatus = 0 AND branch_id" .
            " IN ($sBranch)) {$this->otherSummaryCond} GROUP BY team_id";
