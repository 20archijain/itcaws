...
class BillCutReport
{
    ...
    public function getDownloadData()
    {
        ...
        // handle single or both team types properly
        ...
        foreach ($branch as $branchId) {
            ...
            $sProductQuery = "
            SELECT DISTINCT rec_id, product_name, summary_column_name, team_type
            FROM $branchProductsTable
            WHERE dstatus = 0 AND branch_id = $branchId $productCond $teamTypeCond
            ORDER BY team_type, product_name
        ";
            ...
            if ($iProductRows > 0) {
                while ($rowProduct = $this->_dbConn->GetData($sProductAction)) {
                    ...
                    $arrProductDetails = getRowColumns(
                        $this->_dbConn,
                        $branchProductsTable,
                        "category_name, is_focusbrand",
                        "dstatus = 0 AND rec_id = $recId"
                    );
            ...
            $sQuery = "
            SELECT a.capture_datetime, a.ques_0, b.team_id, b.team_name, b.is_type,
                   b.wd_code, c.district, c.branch_name, c.main_branch
            FROM $respTable AS a, $projectTeamTable AS b, $branchTable AS c
            WHERE a.dstatus = 0 AND a.team_id = b.team_id
              AND b.branch_id = c.branch_id
              AND a.ques_0 IN ('Outlet Order','Add Outlet')
              $where $whereFilter AND b.branch_id = $branchId $Cond
            GROUP BY a.team_id
