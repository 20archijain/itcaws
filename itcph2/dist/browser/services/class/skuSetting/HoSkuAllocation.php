...
class HoSkuAllocation
{
    ...
    final public function getSubmittedData()
    {
        ...
        $rsAction = null;
        $iActionRows = 0;
        $query = "select category_name, product_name, dspm_focus, is_focusbrand from tblbranch_pickupstock_products_allocation where dstatus = 0 AND month = '$month' AND year = '$year' AND branch_id = '$region' AND team_type = '$teamType' AND filled_by_ho = 1";
        // echo $query;die;
    ...
    final public function getSelectedRecord()
    {
        ...
        $rsAction = null;
        $iActionRows = 0;
        $query = "select category_name, product_name, dspm_focus, is_focusbrand, summary_column_name from tblbranch_pickupstock_products_allocation where dstatus = 0 AND month = '$month' AND year = '$year' AND branch_id = '$region' AND team_type = '$teamType' AND filled_by_district = 1";
        // echo $query;die;
        $this->_dbConn->ExecuteSelectQuery($query, $rsAction, $iActionRows);

        if ($iActionRows > 0) {
            while ($row = $this->_dbConn->GetData($rsAction)) {
                if (isset($row['dspm_focus']) && $row['dspm_focus'] == 1) {
    ...
    final public function submitData()
    {
        ...
        foreach ($selectedProducts as $record) {
            ...
            if ($rec_id > 0) {
                updateRecord($this->_dbConn, "tblbranch_pickupstock_products_allocation", "dspm_focus = ?, is_focusbrand = ?, filled_by_ho = ?, user_id = ?", "rec_id = $rec_id", [$dspm_focus, $is_focusbrand, 1, $user]);
