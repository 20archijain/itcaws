...
class AddFocusBrand
{
    ...
    final public function addDataForFocus()
    {
        ...
        $sAction = null;
        $iRows = 0;
        $sQuery = "SELECT distinct branch_id FROM tblbranch_pickupstock_products_history WHERE dstatus = 0 AND month = '$prevMonth' AND year = '$prevYear'";
        ...
        if ($iRows > 0) {
            while ($row = $this->_dbConn->GetData($sAction)) {
                ...
                $sAction1 = null;
                $iRows1 = 0;
                $sQuery1 = "SELECT json_id, team_type, is_focusbrand, category_name, product_name, summary_column_name, net_rate, sort_order, pkt_size FROM tblbranch_pickupstock_products_history WHERE dstatus = 0" .
                ...
                if ($iRows1 > 0) {
                    while ($row1 = $this->_dbConn->GetData($sAction1)) {
                        ...
                        if ($recId == 0) {
                            addRecord($this->_dbConn, "tblbranch_pickupstock_products_history", "json_id, team_type, is_focusbrand, category_name, product_name, summary_column_name, net_rate, sort_order, pkt_size, branch_id, month, year", "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?", $arrList);
                        }
                    };
                }
            }
