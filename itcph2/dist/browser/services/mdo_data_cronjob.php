...
class ProcessMdoData
{
    ...
    final public function processMdoData()
    {
        ...
        $type = [0 => "VAN DS", 1 => "Niche", 2 => "Town SWD", 3 => "Hybrid", 4 => "SCP", 5 => "NPSR", 6 => "RMD", 8 => "SCP DS", 9 => "Common FMCG Lite DS"];
        $rsAction = null;
        $iRows = 0;
        ...
        // $sQuery = "SELECT id, mdo_id, teams, is_type FROM tblmdo_access WHERE dstatus = 0 AND is_type NOT IN (2,5) AND mdo_id IN (22854) AND is_previous_day_updated = 0 LIMIT 50";
        ...
        if ($iRows > 0) {
            while ($row = $this->dbConn->GetData($rsAction)) {
                ...
                foreach ($getRouteDetails as $row) {
                    ...
                    $iStatus = isRecordExist($this->dbConn, "tblmdo_offline_data", "id", "team_id = ? AND ds_id = ? AND outlet_id = ?", [$mdoId, $team, $shopId]);
