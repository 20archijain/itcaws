...
class BranchSkuAllocation
{
    ...
    final public function getSubmittedData(){
        ...
        $rsAction = null;
        $iActionRows = 0;
        $query = "select category_name, product_name, dspm_focus, is_focusbrand from tblbranch_pickupstock_products_allocation where dstatus = 0 AND month = '$month' AND year = '$year' AND branch_id = '$region' AND team_type = '$teamType' AND filled_by_branch = 1";
        // echo $query;die;
        $this->_dbConn->ExecuteSelectQuery($query, $rsAction, $iActionRows);
