...
class DistrictSkuAllocation
{
    ...
    final public function submitData()
    {
        ...
        foreach ($selectedProducts as $record) {
            ...
            if ($rec_id > 0) {
                updateRecord($this->_dbConn, "tblbranch_pickupstock_products_allocation", "dspm_focus = ?, is_focusbrand = ?, filled_by_district = ?", "rec_id = $rec_id", array($dspm_focus, $is_focusbrand, 1));

                 $arrStatus[] = 2;
            } else {
                $cols = "month, year, branch_id, team_type, dspm_focus, is_focusbrand, category_name, product_name, summary_column_name, filled_by_district, rcd, rdt";
                $vals = "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?";
