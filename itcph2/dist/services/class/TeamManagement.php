...
class TeamManagement
{
    ...
    private function checkAddTeamValidation(
        ...
        if ($addMethodType == 1) {
            if (isNonEmptyArray($teams)) {
                ...
                $obj->addValidation('dsName', "", 'alnum_s_u_h', 1, 1, $this->_validationLength['TEAM_NAME_MAXLENGTH'], 'Surveyor Name');
                if ($jsonId != '100' && $jsonId != '101') {
                    $obj->addValidation('dsPhone', "", 'mobile', 1, $this->_validationLength['MOBILE_MINLENGTH'], $this->_validationLength['MOBILE_MAXLENGTH'], 'Surveyor Phone');
                }
            }
        } else {
            // Using Index
            $obj->addValidation('dsName', $dsName, 'alnum_s_u_h', 1, 1, $this->_validationLength['TEAM_NAME_MAXLENGTH'], 'Surveyor Name');
    ...
    final public function addTeam()
    {
        ...
        if ($jsonId == '4') {
            $jsonId = 99; //It is beacuse Json Id Of NPSR is also 99
        }
