...
export class ProductiveAccordionComponent implements OnChanges {
  ...
  downloadLevels = ['district', 'branch', 'circle', 'section', 'wd', 'team'];
  ...
  downloadVisibleDataAsExcel(_label: string): void {
    ...
    if (_label == 'district') {
      this.monthlySalesData.districtData.forEach((district: any) => {
        rows.push(this.createRow({ district: district.district }, district.districtLevelSale));
      });
    }
    else if (_label == 'branch') {
      this.monthlySalesData.districtData.forEach((district: any) => {
        district.branchData.forEach((branch: any) => {
          rows.push(this.createRow({ district: district.district, branch: branch.branch_name }, branch.branchLevelSale));
      ...
      this.monthlySalesData.districtData.forEach((district: any) => {
        district.branchData.forEach((branch: any) => {
          branch.circleData.forEach((circle: any) => {
            rows.push(this.createRow({ district: district.district, branch: branch.branch_name, circle: circle.circle }, circle.circleLevelSale));
          });
        });
      });
    }
      ...
      this.monthlySalesData.districtData.forEach((district: any) => {
        district.branchData.forEach((branch: any) => {
          branch.circleData.forEach((circle: any) => {
            circle.sectionData.forEach((section: any) => {
              rows.push(this.createRow({ district: district.district, branch: branch.branch_name, circle: circle.circle, section: section.section }, section.sectionLevelSale));
            });
          });
        });
      });
      ...
      this.monthlySalesData.districtData.forEach((district: any) => {
        district.branchData.forEach((branch: any) => {
          branch.circleData.forEach((circle: any) => {
            circle.sectionData.forEach((section: any) => {
              section.wdData.forEach((wd: any) => {
                rows.push(this.createRow({ district: district.district, branch: branch.branch_name, circle: circle.circle, section: section.section, wd: wd.wd_code }, wd.wdLevelSale));
              });
            });
          });
        });
