...
class MdoTargetReport
{
    ...
    foreach ($arrMonth as $month) {
      ...
      while ($row = $this->_dbConn->GetData($sAction)) {
        ...
        foreach ($gateParameters as $paramLabel => $paramConfig) {
          ...
          $gateResults[$paramLabel] = $gateAchieved;
  ...
  final public function getBranchList($cond = "")
  {
    ...
    $rsAction    = null;
    $iActionRows = 0;
    $query = "SELECT DISTINCT s.main_branch, s.branch_name FROM mdo_dspm_summary AS s WHERE s.branch_name != '' AND s.main_branch != '' AND $where ORDER BY s.branch_name";
    $this->_dbConn->ExecuteSelectQuery($query, $rsAction, $iActionRows);
